...
class KiPool extends Component 
{
  ...
  async loadBlockchainData() 
  {
    ...
    // Load KiPool
    const kiPoolData = KiPoolAbi.networks[networkId]
    let kiPool
    
    if(kiPoolData) 
    {
      kiPool = new web3.eth.Contract(KiPoolAbi.abi, kiPoolData.address)
      ...
      const allowance = await this.state.goToken.methods.allowance(this.state.account, kiPoolData.address).call()
